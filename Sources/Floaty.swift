...
open class Floaty: UIView {
    ...
    private var myInitialFrame: CGRect = .zero
    ...
    @objc open func open() {
        ...
        myInitialFrame = self.frame
        if let window = UIApplication.shared.keyWindow {
            self.removeFromSuperview()
            let newFrame = mySuperView!.convert(self.frame, to: window)
            self.frame = newFrame
    ...
    @objc open func close() {
        ...
        if(items.count > 0){
            ...
            UIView.animate(withDuration: 0.3, delay: 0,
                ...
                let newFrame = self.superview!.convert(self.frame, to: self.mySuperView!)
                self.removeFromSuperview()
                self.frame = newFrame
    ...
    private var needFrameUpdate = true
